...
EOF
  ...
  def self.demographics_report(patient_type, start_date, end_date)
    ...
    query = "SELECT person_name.given_name AS first_name, person_name.family_name AS surname, " +
              "person_attribute.value AS nearest_health_center, person.gender AS patient_gender,  " +
              "(YEAR(patient.date_created) - YEAR(person.birthdate)) AS age, " +
              "person.birthdate AS birthdate, DATE(patient.date_created) AS start_date " +
              extra_parameters +
            "FROM person_attribute, patient, person, person_name " + sub_query +
            "WHERE patient.patient_id = person.person_id " +
              "AND person.person_id = person_attribute.person_id " +
              "AND person.person_id = person_name.person_id " + extra_conditions +
            ...
            "ORDER BY person_name.family_name, patient.date_created"

    patients_data = Patient.find_by_sql(query)
